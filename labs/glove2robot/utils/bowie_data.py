...
from glove2robot.utils.constants import HAND_KEYPOINTS, ALLEGRO_JOINTS
...
class BowieSyncData:
    ...
    def __init__(self, filename, DATA_PATH, verbose=False):
        print("Correctly initializing BowieSyncData...") #DOES NOT POINT TO THIS ONE
        ...
        rs_ts, rs_color, left_ir, right_ir, rs_depth, bowie_data_array = self.open_bowie_sync_pkl(self.PATH, self.verbose)
        # rs_ts, rs_color, left_ir, right_ir, rs_depth, bowie_data_array = self.get_bowie_sync_data(self.PATH, self.verbose)
        # self.realsense_data = realsense_data
        # self.bowie_data = bowie_data

        # (rs_time, rs_color, rs_depth, rs_ir1, rs_ir2, bowie_data_array) = self.get_bowie_sync_data(
    ...
    def open_bowie_sync_pkl(self, path, verbose=False):
        ...
        rs_color = np.asarray(pickle.load(open(path + "/rgbs.pkl", "rb")))
        rs_depth = np.asarray(pickle.load(open(path + "/depth.pkl", "rb")))
        left_ir = np.asarray(pickle.load(open(path + "/left_ir.pkl", "rb")))
        right_ir = np.asarray(pickle.load(open(path + "/right_ir.pkl", "rb")))
        bowie_data = np.asarray(pickle.load(open(path + "/synced_mags.pkl", "rb")), dtype="object")
        bowie_data = bowie_data[:,1] 
        # rs_ts, rs_color, left_ir, right_ir, rs_depth, bowie_data_array
        import ipdb; ipdb.set_trace()
        return rs_ts, rs_color, left_ir, right_ir, rs_depth, bowie_data
        # return rs_ts, rs_color, bowie_data
    ...
    def get_bowie_sync_data(self, realsense_data, bowie_data, verbose=False):
        ...
        assert len(realsense_data[0]) == 5
        rs_time, rs_color, rs_depth, rs_ir1, rs_ir2 = (
            [item[0] for item in realsense_data],
            [item[1] for item in realsense_data],
            [item[2] for item in realsense_data],
            [item[3] for item in realsense_data],
            [item[4] for item in realsense_data],
        ...
        rs_ir1 = np.array(rs_ir1)
        rs_ir2 = np.array(rs_ir2)
        ...
        return rs_time, rs_color, rs_depth, rs_ir1, rs_ir2, bowie_data_array
    ...
    def plot_ema_binary(self, sensor_idx, threshold, history, alpha, plot=True):
        ...
        if plot:
            ...
            plt.suptitle(f"sensor idx: {sensor_idx}, ema α={alpha}")
            plt.show()
